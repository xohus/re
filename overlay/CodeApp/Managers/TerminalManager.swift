...
class TerminalManager: ObservableObject {
    ...
    /// Sends a command to the interactive shell attached to the current SFTP workspace.
    /// Local iPad terminals cannot launch desktop binaries such as Codex, so XCode
    /// uses the existing SSH shell when a remote workspace is connected.
    @discardableResult
    func runCommandInRemoteTerminal(_ command: String) -> Bool {
        assertMainThread()

        let command = command.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !command.isEmpty,
            let terminal = remoteTerminal,
            let provider = terminal.terminalServiceProvider,
            let data = "\(command)\n".data(using: .utf8)
        else {
            return false
        }

        setActiveTerminal(id: terminal.id)
