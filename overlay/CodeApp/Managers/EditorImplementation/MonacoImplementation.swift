...
extension MonacoImplementation: EditorImplementation {
    ...
    func getFullValue() async -> String {
        return
            (try? await monacoWebView.evaluateJavaScriptAsync(
    ...
    func _runEditorAction(id: String) async {
        guard let encoded = id.base64Encoded() else { return }
        _ = try? await monacoWebView.evaluateJavaScriptAsync(
            "editor.getAction(decodeURIComponent(escape(window.atob('\(encoded)'))))?.run()")
    }
}
